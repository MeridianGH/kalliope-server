...
export function preventSimilarColor(color, reference, brighten = false) {
    function changeColor(color, reference, brighten, recursionDepth = 0, originalColor = color) {
        const stepSize = 20;
        const maxDepth = Math.ceil(255 / stepSize);
        if (recursionDepth + 1 >= maxDepth) {
            return changeColor(originalColor, reference, !brighten);
        }
...
export async function findDominantColor(url) {
